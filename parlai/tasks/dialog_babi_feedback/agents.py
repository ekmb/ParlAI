...
#
# which specifies task 2, and policy with 0.5 answers correct
...
def _path(task, opt):
    ...
    prefix = os.path.join(opt['datapath'], 'dialog-bAbI-feedback', 'dialog-bAbI-feedback-tasks')
    suffix = ''
...
class TaskTeacher(FbDialogTeacher):
    ...
    def setup_data_with_feedback(self, path):
        ...
        The detailed description is available in the FbDialogTeacher setup_data().
        
        The difference with the above method is that here feedback is appended to the query of the current example,
        in the default setup the feedback is appended to the x from the next example.
        To append the feedback to the current example, modify task name like this:
        python examples/display_data.py -t dbll_babi:task:2_p0.5:f - parsing with feedback that immediately follows query
        python examples/display_data.py -t dbll_babi:task:2_p0.5 - default parsing
