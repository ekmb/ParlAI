...
class MemnnCombAgent(Agent):
    ...
    def add_cmdline_args(argparser):
        ...
        arg_group.add_argument('--hops', type=int, default=1,
    ...
    def predict(self, xs, answer_cands, ys=None, feedback_cands=None):
        ...
        path = 'data/dialog-bAbI-feedback/dialog-bAbI-feedback-tasks/dialog-babi-candidates.txt'
        all_cands = []
        with open(path) as f:
            for line in f:
                all_cands.append(line[2:].strip())
        ...
        if is_training:
            ...
            if self.model_setting == 'FP' or self.model_setting == 'FP_after_rbi':
                update_params = False
    ...
    def batchify(self, obs):
        ...
        self.eval_labels = [random.choice(ex['eval_labels']) for ex in exs if 'eval_labels' in ex]
